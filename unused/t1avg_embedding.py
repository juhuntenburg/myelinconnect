...
for hemi in hemis: 
    ...
    for affine_method in affine_methods:
        ...
        affinity_file = '/scr/ilz3/myelinconnect/all_data_on_simple_surf/embed/t1avg/%s_%s_t1avg_affinity_%s.hdf5'%(hemi, smooth, affine_method)
        embed_file = '/scr/ilz3/myelinconnect/all_data_on_simple_surf/embed/t1avg/%s_%s_t1avg_embedding_%s_%s.npy'%(hemi, smooth, str(n_embedding), affine_method)
        embed_dict_file = '/scr/ilz3/myelinconnect/all_data_on_simple_surf/embed/t1avg/%s_%s_t1avg_embedding_dict_%s_%s.pkl'%(hemi, smooth, str(n_embedding), affine_method)
        ...
        if affinity:
            #print 'chebychev'
            t1avg = np.mean(t1[:,3:8], axis=1)
            t1avg = t1avg[:,np.newaxis]
            #coeff, poly = chebapprox(t1_3_7, degree=4)
            #coeff = t1.copy()
            print 'affinity'
            t1_3_7_affine = dist.compute_affinity(t1avg, method=affine_method)

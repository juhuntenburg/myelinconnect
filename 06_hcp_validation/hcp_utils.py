def load_hcp(surf_dir):
    
    import numpy as np
    import nibabel as nib
    
    surfmL = nib.freesurfer.read_geometry(surf_dir+'S900.L.midthickness_MSMAll.32k_fs_LR.surf')
    surfiL = nib.freesurfer.read_geometry(surf_dir+'S900.L.very_inflated_MSMAll.32k_fs_LR.surf')
    surffL = nib.freesurfer.read_geometry(surf_dir+'S900.L.flat.32k_fs_LR.surf')
    surfL = []
    surfL.append(np.array(surfmL[0]*0.3 + surfiL[0]*0.7))
    surfL.append(surfmL[1])
    
    surfmR = nib.freesurfer.read_geometry(surf_dir+'S900.R.midthickness_MSMAll.32k_fs_LR.surf')
    surfiR = nib.freesurfer.read_geometry(surf_dir+'S900.R.very_inflated_MSMAll.32k_fs_LR.surf')
    surffR = nib.freesurfer.read_geometry(surf_dir+'S900.R.flat.32k_fs_LR.surf')
    surfR = []

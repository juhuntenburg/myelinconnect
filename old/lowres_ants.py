...
#     # binarize and dilate highres anat to mask epi
...
#                      name='brainmask')
    ...
    nonlinear.connect([(inputnode, brainmask, [('brain_mask', 'input_image'),
                       ...
                       (inputnode, mask_epi, [('epi2highres', 'in_file')])
                       ])
    ...
    downsample_t1 = Node(afni.Resample(outputtype='NIFTI_GZ',
                                       resample_mode='Li',
                                       voxel_size=(1.5,1.5,1.5)),
                         name='downt1')
    
    downsample_epi = Node(afni.Resample(outputtype='NIFTI_GZ',
                                       resample_mode='Li'),
                          name='downepi')
    
    nonlinear.connect([(mask_t1, downsample_t1, [('out_file', 'in_file')]),
                       (mask_epi, downsample_epi, [('out_file', 'in_file')]),
                       (downsample_t1, downsample_epi, [('out_file', 'master')])])
    ...
    # connections
    nonlinear.connect([(downsample_epi, antsreg, [('out_file', 'moving_image')]),
                       (downsample_t1, antsreg, [('out_file', 'fixed_image')]),
